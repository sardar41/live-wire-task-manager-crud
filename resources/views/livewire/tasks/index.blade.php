<div class="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
    <div class="flex justify-between items-center mb-4">
        <!-- Search Input Field -->
        <input type="text" wire:model.live.debounce.300ms="search" class="border border-gray-300 rounded-lg p-2 w-1/3" placeholder="{{ __('Search tasks by title...') }}">
    ...
    <div class="bg-gray-100 p-4 rounded-lg">
        @if($tasks->isEmpty())
            <p class="text-gray-500 text-center">{{ __('No tasks found.') }}</p>
        @else
                ...
                <table class="w-full border-collapse border border-gray-300">
                    <thead>
                        <tr class="bg-gray-200 text-gray-700">
                            ...
                            <th class="border border-gray-300 px-4 py-2 text-left">{{ __('Description') }}</th>
                            <th class="border border-gray-300 px-4 py-2 text-left">{{ __('Status') }}</th>
                            @if (Auth::user()->user_type == 'admin')
                                <th class="border border-gray-300 px-4 py-2 text-left">{{ __('Username') }}</th>
